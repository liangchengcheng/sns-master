<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8" %>
<%@ page import="com.lcc.osf.model.User" %>
    ...
    <c:forEach items="${comments }" var="comment">
        <div class="comment" id="comment${comment.id }" author="${comment.comment_author }">
            ...
            <div class="content">
                ...
                <c:if test="${comment.comment_parent != 0 }">
                    <a class="author"
                       href="<c:url value="/user/${comment.comment_author }" />">${comment.comment_author_name }</a> 回复
                    <a class="author"
                       href="<c:url value="/user/${comment.comment_parent_author }" />">${comment.comment_parent_author_name }</a>
                </c:if>
