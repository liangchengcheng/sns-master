...
import com.lcc.osf.dao.EventDAO;
...
public class EventDAOImpl implements EventDAO {
    ...
    public List<Event> getEventsWithRelations(Map<Integer, List<Integer>> relations) {
        StringBuffer sql = new StringBuffer("select * from " + TABLE + " where ");

        int i = 0;
        HashMap<String, Object> paramMap = new HashMap<String, Object>();
        for (Integer type : relations.keySet()) {
            if (i != 0) {
                sql.append(" or ");
            }
            sql.append("(object_type=" + type + " and object_id in (:" + String.valueOf(type) + i + "))");
            paramMap.put(String.valueOf(type) + i, relations.get(type));
            i++;
        }
    ...
    public Event getEvent(final int object_type, final int object_id) {
        ...
        return jdbcTemplate.query(new PreparedStatementCreator() {
            ...
            public Event extractData(ResultSet resultSet) throws SQLException, DataAccessException {
                if (resultSet.next()) {
                    return generateEvent(resultSet);
                }
                return new Event();
            }
        });
    ...
    public List<Event> getEvents(int start, int step) {
        ...
        return jdbcTemplate.query(sql, new Object[]{start, step}, new RowMapper<Event>() {
